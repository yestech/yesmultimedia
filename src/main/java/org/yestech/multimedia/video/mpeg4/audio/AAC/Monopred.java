...
public final class Monopred {
	...
	static void reset_pred_state(PRED_STATUS psp) {
		int m, grad;
		grad = GRAD;
		for(m=0;m<=grad;m++)	psp.r[m] = 0;
		for(m=0;m<=grad;m++)	psp.kor[m] = 0;
		for(m=0;m<=grad;m++)	psp.var[m] = 1;
		for(m=0;m<=grad;m++)	psp.k[m] = 0;
	}
	 ...
	 ***										
	 ********************************************************************************/
	static void init_pred_stat(PRED_STATUS psp, int grad, float alpha, float a, float b) throws IOException {
		/* Test of parameters */

		if((grad < 0) || (grad > Constants.MAX_PGRAD)) {
			System.out.println("\n\n ****** error in routine init_pred_stat ******");
			System.out.println("\nwrong predictor order: " + grad);
			System.out.println("range of allowed values: 0 ... " + Constants.MAX_PGRAD + " (=MAX_PGRAD)\n");
	 ...
	 ***										
	 ***    parameters:	lqs:	last quantised sample				
	...
	static float monopred(float lx, PRED_STATUS psp) {
		 ...
		 *	and the old r-values stored in the registers in order to reconstruct the
		 *	predictor status of the previous step (instead of storage in state variables!)
		 */
	...
	static void predict(Info info, int profile, int[] lpflag, PRED_STATUS[] psp, float[] prev_quant, float[] coef) throws IOException
	{
		int j, k, b, to, flag0;
		short[] top;
		int lp_flag_index = 0;
		...
		if (profile != Constants.Main_Profile) {
			if (lpflag[0] != 0) {
				throw new IOException("Prediction isn't allowed in this profile!");
			} else {
...
//			fltcpy(prev_quant, coef, Constants.LN2); @TODO check this code
	 ...
	 ***										
	 ********************************************************************************/
	static void predict_reset(Info info, int[] prstflag, PRED_STATUS[][] psp, float[][] prev_quant,
...
/*				
				if (AACDecoder.debug[Constants.DEBUG_R]) {
					System.out.print("PRST: prstgrp: " + prstgrp + "  prstbits: ");
					for (j=Constants.LEN_PRED_RSTGRP-1; j>=0; j--) {
			   			System.out.print(prstflag[j] + " ");
			   		}
