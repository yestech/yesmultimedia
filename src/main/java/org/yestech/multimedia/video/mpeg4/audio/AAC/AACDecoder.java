		...
		for (i = 0; i < info.nsbk; i++) {
			if (info.bins_per_sbk[i] > Constants.SN2) {
				...
				if (first_short) {
					...
					if((j = (int)audio_stream.next_bits(1)) == 0) {
...
/*		if( debug[Constants.DEBUG_G] ) 
	...
	getmask(Info info, byte[] group, byte max_sfb, byte[] mask) throws IOException {
		int b, i, mp;
		int group_index = 0, max_sfb_index = 0, mask_index = 0;
...
//		restarttio();
	...
	int getdata(byte[] data_bytes) throws IOException
	{
		...
		d_tag = (int)audio_stream.next_bits(Constants.LEN_TAG);
...
/*		if (debug[Constants.DEBUG_X]) {
			System.out.println("data element " + d_tag + " has " + d_cnt + " bytes");
		}	*/
	
		if (byte_align_flag) {
			audio_stream.byteAlign(); // TODO check this code
		}
